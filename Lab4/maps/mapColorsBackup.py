    ...
    if msg: 
		...
		for x in range(0,numCells):
		    ...
		    if isFrontier(x):
		        ...
		        point.x = (float(x % numCols) * cell_size) + .66#column number * meters/cell
		        #print str(point.x)
		        point.y = (float(x // numCols) * cell_size) + .15#row number * meters/cell
		        #print str(point.y)
...
def expandPoint(x,y):
    ...
    for i in range(-1, 2):
        for j in range(-1, 2):
			...
			index = pointToIndex(pnt.x/.3,pnt.y/.3)
			l = len(mapdata.data)
			#check if in bounds
...
def expansion(cells):
	...
	expanded_gridcells.cell_width = .3
	expanded_gridcells.cell_height = .3
