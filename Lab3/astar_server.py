...
################## A star service written for lab 3 Extra Credit ##########################
from nav_msgs.srv import GetPlan

def handle_astar(req):
	print "Returning Path From:"
	print str(req.a)
	print "To:"
	print str(req.b)
	return astarResponse(astar(req.a,req.b))

def astar_server():
	rospy.init_node('astar_server')
	s = rospy.Service('astar', GetPlan, handle_astar)
	print "READY FOR ASTAR COORD!"
	rospy.spin()
...
def astarSub():		
	global start
	global goal											# returns zero if no path from start to goal otherwise returns path from start to goal
	global openSet
	global closedSet
